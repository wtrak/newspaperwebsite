...
- Image-backed Library of Congress bulk catalog with 2,000+ real issue pages, source links, and issue metadata
...
The browse count includes only records with a real archive image and source link. Custom research requests are displayed and counted separately from front pages. The catalog builder reads the Library of Congress bulk NDNP issue manifests instead of making a separate search request for every date, and the storefront reveals large result sets 48 pages at a time.

```bash
npm run catalog:build
npm run catalog:build:seed
